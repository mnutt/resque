...
Raphael.fn.g.barchart = function (x, y, width, height, values, opts) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < (multi || 1); j++) {
            ...
            var shadowHeight = (top - y - barvgutter) || height - 2 * barvgutter;
            var shadow = this.rect(bar.x - bar.w / 2, y + barvgutter, barwidth, shadowHeight)
            shadow.attr({stroke: "none", fill: "90-#EEEEEE-#F5F5F5"});
        }
        if (opts.stacked) {
            var cvr;
