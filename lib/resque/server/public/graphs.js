...
Resque.GraphData = function(data, options) {
    ...
    var utc = new Date(localTime.getTime() - localOffset);

    this.currentTime = this.beginningOfIncrement(utc);
    this.axes = [];
    this.sets = [];
    this.setNames = [];
    ...
    for(var i = 0; i < this.options.numberOfBars; i++) {
        ...
        var label = (new Date(time.getTime() + localOffset)).timeLabel(this.timeUnit);
